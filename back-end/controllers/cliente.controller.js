...
export const listarClientes = async (req, res) => {
  try {
    let { page = 1, limit = 10, nome, email, telefone } = req.query;
  ...
  try {
    ...
    if (email) {
      ...
      if (emailExiste && emailExiste.idCliente !== idCliente) {
        return res
          .status(400)
          .json({ message: "Email já está em uso por outro cliente." });
      }
    }
    ...
    if (telefone) {
      ...
      if (telefoneExiste && telefoneExiste.idCliente !== idCliente) {
        return res
          .status(400)
          .json({ message: "Telefone já está em uso por outro cliente." });
      }
    }
